...
import {
  PlaywrightAdapter,
  PlaywrightConfig,
} from '../utils/PlaywrightAdapter';
import {
  RobotsChecker,
  RobotsConfig,
  CrawlPermission,
} from '../utils/RobotsChecker';
import {
  NewsArticleValidator,
  ValidationConfig,
  ValidationResult,
} from '../utils/NewsArticleValidator';

export class NewsCrawler extends BaseCrawler {
  private newsConfig: NewsSourceConfig;
  private discoverySession?: DiscoverySession;
  private playwrightAdapter?: PlaywrightAdapter;
  private robotsChecker: RobotsChecker;
  private articleValidator: NewsArticleValidator;
  ...
  constructor(
    ...
    if (playwrightConfig || this.newsConfig.config?.['enableDynamicContent']) {
      this.playwrightAdapter = new PlaywrightAdapter(playwrightConfig, logger);
    }

    // Initialize robots checker
    const defaultRobotsConfig: RobotsConfig = {
      userAgent:
        (this.newsConfig.config?.['userAgent'] as string) ||
    ...
    this.articleValidator = new NewsArticleValidator(
   ...
   */
  private validateNewsConfig(): void {
   ...
   */
  protected override getRateLimitConfig(): RateLimitConfig {
    return {
      maxRequests: 5,
  ...
  private parseDate(dateString: string): Date | undefined {
    ...
    try {
      const isoDate = parseISO(cleanDateString);
      if (isValid(isoDate)) {
        return isoDate;
      }
    } catch (error) {
  ...
  private extractText(
    $: cheerio.CheerioAPI,
    selector: string | cheerio.Cheerio<any>,
    fallback = ''
  ): string {
   ...
   */
  private extractMetadata($: cheerio.CheerioAPI, selectors: string[]): string {
   ...
   */
  private extractImageUrl($: cheerio.CheerioAPI, element: any): string {
    const $el = $(element);
    return (
   ...
   */
  private async shouldUseDynamicContent(
    url: string,
    initialHtml?: string
  ): Promise<boolean> {
  ...
  private async getPageContent(url: string): Promise<string> {
    ...
    const staticHtml = (await this.makeRequest(url)) as string;
      ...
      if (structuredData.length > 0) {
        try {
          ...
          if (jsonLd.keywords && Array.isArray(jsonLd.keywords)) {
            ...
            tags.push(
              ...jsonLd.keywords.split(',').map((tag: string) => tag.trim())
            );
          }
        } catch (error) {
   ...
   */
  private initDiscoverySession(baseUrl: string): void {
   ...
   */
  private extractArticleUrls($: cheerio.CheerioAPI, baseUrl: string): string[] {
    const { selectors } = this.newsConfig;
    const urls: string[] = [];
   ...
   */
  private extractCategorySectionLinks(
    $: cheerio.CheerioAPI,
    baseUrl: string
  ): void {
  ...
  public async discoverArticles(
    ...
    const result: ArticleDiscoveryResult = {
      urls: [],
      currentPage: 1,
      hasMorePages: false,
    };
   ...
   */
  public async discoverArticlesDeep(startUrl: string): Promise<string[]> {
   ...
   */
  private async checkRobotsCompliance(url: string): Promise<CrawlPermission> {
   ...
   */
  static fromPreset(
    presetName: keyof typeof KOREAN_NEWS_PRESETS,
    url: string,
    logger?: winston.Logger,
    playwrightConfig?: PlaywrightConfig,
    robotsConfig?: RobotsConfig,
    ...
    const preset = KOREAN_NEWS_PRESETS[presetName];
    if (!preset) {
  ...
  static getAvailablePresets(): string[] {
    return Object.keys(KOREAN_NEWS_PRESETS);
  }
}
