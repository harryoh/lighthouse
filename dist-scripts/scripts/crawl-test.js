...
const NaverNewsCrawler_1 = require('../libs/crawler-core/src/crawlers/NaverNewsCrawler');
async function testCrawl() {
  ...
  try {
    ...
    const crawler = new NaverNewsCrawler_1.NaverNewsCrawler(
      testUrl,
      undefined, // logger
    ...
    for (let i = 0; i < articlesToShow; i++) {
      ...
      if (article.metadata?.tags && Array.isArray(article.metadata.tags)) {
        console.log(`🏷️  Tags: ${article.metadata.tags.join(', ')}`);
      }
    }
    // Show statistics
    console.log('\n' + '='.repeat(60));
